 ...
 <a style="display:none;" href="javascript:void(0);" class="checkLoginSession" data-href="<?php echo $this->config->item("checkLoginSession");?>">checkLoginSession</a>
    ...
    <script>
		...
		function checkLogin()
		{
			$(".checkLoginSession").ventricleDirectAjax("GET",'','checkLoginSessionResponse');
			setTimeout(function() {
			  checkLogin();
            }, 3000);
		}
		...
		function checkLoginSessionResponse(response)
		{
			if( (typeof response === "object") && (response !== null) )
			{
				if(!response.success)
				{
					window.location.href = "<?php echo $this->config->item("loginCtrl");?>";
				}
				else
				{
